...
class APITestCase(unittest.TestCase):
    ...
    @patch('app.pizzaApi.check_database_connection', return_value="OK")
    @patch('app.pizzaApi.check_message_queue', return_value="OK")
    ...
    @patch('app.pizzaApi.verify_service_token', return_value=False)
    def test_order_unauthorized(self, mock_check_auth):
        response = self.app.post("/order", json={})
        self.assertEqual(response.status_code, 401)

    @patch('app.pizzaApi.verify_service_token', return_value=True)
    @patch('app.pizzaApi.add_order_to_queue')
