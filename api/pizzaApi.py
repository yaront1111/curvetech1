...
class Config:
    ...
    MONGO_URL = f'mongodb://{MONGO_USERNAME}:{MONGO_PASSWORD}@pizza-ordering-mongodb:27017/pizza_db'
...
def check_message_queue():
    try:
        ...
        connection_params = pika.ConnectionParameters(app.config['RABBITMQ_URL'], credentials=credentials)
...
def add_order_to_queue(order):
    connection = pika.BlockingConnection(pika.ConnectionParameters(host=app.config['RABBITMQ_URL']))
